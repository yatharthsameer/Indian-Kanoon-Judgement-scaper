...
class IndianKanoonScraper:
    ...
    def scrape_court(self, court_data):
        ...
        # --- NEW CHROME OPTIONS & DRIVER PATH ---
        chrome_opts = Options()
        # Tell Chrome where the browser binary is (the one in your ~/chrome folder)
        chrome_opts.binary_location = (
            "/home/du0/20CS30059/chrome/opt/google/chrome/chrome"
        )
        # You can add more Chrome arguments if desired:
        # chrome_opts.add_argument("--headless")  # If you want headless mode

        try:
            # Provide the local chromedriver path, and the custom Chrome options
            # UC = "undetected-chromedriver" mode to bypass basic bot detection
            with SB(
                uc=True,
                browser="chrome",
                driver_path="/home/du0/20CS30059/chrome/chromedriver",
                options=chrome_opts,
            ) as sb:
                # Loop over years
